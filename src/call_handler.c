...
static void call_dist_predictive(json_t* j_camp, json_t* j_plan, json_t* j_chan);
...
void cb_call_distribute(unused__ evutil_socket_t fd, unused__ short what, unused__ void *arg)
{
    ...
    json_t* j_chan;     // channel info to distribute
    ...
    mem_res = memdb_query("select * from channel where uniq_id = (select chan_uuid from dialing where chan_uuid = (select unique_id from park));");
//    mem_res = memdb_query("select * from channel where channel = (select channel from park);");
    ...
    while(1)
    {
        ...
        ret = asprintf(&sql, "select * from campaign where uuid=\"%s\";",
                json_string_value(json_object_get(j_chan, "camp_uuid"))
                );
        ...
        {
            call_dist_predictive(j_camp, j_plan, j_chan);
        }
